---
name: todoist-login
description: Todoist MCPへのOAuth 2.1認証手順。公式MCP Python SDKでブラウザ認証し、トークンを保存する。
---
...
1. `https://todoist.com/.well-known/oauth-authorization-server` からメタデータ取得
2. RFC 7591 動的クライアント登録（`~/.config/todoist-mcp/client_info.json` に永続化）
3. PKCE + authorization_code フロー開始 → **ブラウザが自動で開く**
4. `http://localhost:3030/callback` で認可コード受信
5. トークン交換 → `~/.config/todoist-mcp/tokens.json` に保存（パーミッション 0600）
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/todoist_cli.py logout
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/todoist_cli.py login
```

## 注意事項

- ポート 3030 が他プロセスで使用されていると失敗する
...
認証結果を報告する:
- 認証状態（成功/失敗）
- トークン保存場所（`~/.config/todoist-mcp/tokens.json`）
