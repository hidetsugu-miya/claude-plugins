...
1. このURLをブラウザで開いてTodoistアカウントで認証してください
2. 認証後、ブラウザのアドレスバーに `localhost:3120/callback?code=...&state=...` というURLが表示されます（ページ自体はエラーになります）
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/todoist_cli.py login --code "<ユーザーが貼り付けたURL>"
```
