...
claude-memのWorker HTTP API（localhost:37777）を使用して永続メモリを検索・取得する。過去のセッション情報、観察、タイムラインの参照に使用する。
...
コンテキスト消費を最小化するため、以下の順序で段階的に絞り込む:

1. **検索（インデックス取得）**: IDとタイトルのみ取得（1件あたり約50-100トークン）
2. **タイムライン**: 検索結果のIDをアンカーにして前後コンテキストを取得
3. **観察取得**: 必要なIDのみ指定してフル詳細を取得。フィルタリングなしの全件取得は禁止
